...
- `internal/ai` — agent langchaingo (`agents.Executor`, max iterasi = `max_iterations`, jeda antar loop = `LoopDelay()` di `Plan`) + **8 tools** (`tools.go`, deklarasi tiru picoclaw): `read_file` (path, offset, length; 64KB max; respons header `[file: base | total | read]` + `[TRUNCATED ... offset=X ...]`/`[END OF FILE ...]`), `write_file` (path, content, overwrite; tanpa overwrite file ada → tolak + arahkan ke append/edit), `list_dir` (path; baris `DIR:`/`FILE:`, dir kosong → `(empty directory)` agar model tidak looping), `edit_file` (path, old_text, new_text — unik, respons `File edited: path`), `append_file` (path, content; respons `Appended to path`), `exec` (action wajib: run/list/poll/read/write/kill/send-keys ala picoclaw — hanya `run` yang diimplementasikan; command, sessionId, keys, data, background, pty, cwd, timeout opsional; output di-cap 20k char) + `telegram_sendfile` (butuh `Agent.Telegram` + `opts.ChatID`, file di-jail, cap 20MB; param path/filename/caption), `telegram_getuser` (tanpa arg = `opts.User` si penanya; `user_id` = fetch live via `getChat`: nama, id, username, bio; keduanya error-value ramah bila tanpa konteks Telegram/CLI). Tiap tool `Run` dibungkus hook `opts.OnTool(name, args)` untuk preview live. `model.go` = `retryModel`: **setiap pemanggilan API selalu streaming** (agent `Plan` selalu pasang `StreamingFunc` → `stream:true`; `Compact` summarize non-streaming) dan **API error di-retry total 5x dengan jeda 2 dtk per model call** (tools yang sudah jalan tidak diulang). `fs.go` = jail workspace (`restrict_workspace`: tolak path absolut di luar workspace + `../` escape; berlaku untuk tools DAN `exec` cwd) + helper `readLocalFile`/`writeLocalFile`/`appendLocalFile`/`listLocalDir`/`editLocalFile` (binary ditolak di read). `exec.go` = shell dengan timeout: default 60 dtk bila agent tidak mengisi `timeout`, clamp maks 300 dtk; output di-capture bounded 20k char + marker `...[truncated]` (buffer tidak pernah lebih dari cap); tiap command di-budget RAM `exec_memory_mb` (watcher RSS grup proses per 200ms via `/proc`, Linux saja, lewat → `SIGKILL` + flag `memory_limited`; `ulimit -v` sengaja TIDAK dipakai karena merusak toolchain Go/Java yang butuh VSZ besar); file yang ditulis command di-cap 100MB via `ulimit -f` (unix, best-effort); timeout → kill process group (unix `setpgid` + `SIGKILL`, lihat `exec_unix.go`/`exec_windows.go`, watcher di `exec_mem_linux.go` + stub `exec_mem_other.go`) agar tidak bocor RAM.
