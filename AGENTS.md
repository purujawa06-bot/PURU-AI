...
- `internal/app` — handler Telegram slim (teks saja, busy-guard per-user, menu terdaftar cuma `/help`/`/clear`/`/token` via `SetMyCommands`; `/token` = `tokenInfo`: % history vs `history_token_limit` menuju summarize, Markdown fallback, >4096 char → file). `processMessage` mengisi `ProcessOptions{ChatID, User, OnTool}`; `previewHook` edit pesan `🤔 ...` live (`🔧 tool arg`, throttle 1.2 dtk, maks 6 baris) bila `tools_preview=true`.
