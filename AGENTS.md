...
- Isi: `telegram_bot_token`, `model{base_url,api_key,model,temperature}`, `workspace` (default `~/.puru/workspace`), `restrict_workspace` (default `true`), `max_iterations` (default `500`), `history_token_limit` (default `30000`).
...
- `internal/messages`, `internal/tokens`, `internal/telegram`, `internal/ai/openai` — dipertahankan (skema history Vercel-compatible, hitung token o200k_base, klien Telegram manual, transport OpenAI streaming).

## Perilaku Penting
- **Satu model, satu attempt**: tanpa fallback provider, tanpa retry storm. Gagal = balasan `Maaf, saya tidak bisa merespons saat ini.` + log `[ai]`.
