---
name: skill-creator
description: Create, update, or review PuruClaw skills. Use when writing a new skill, modifying an existing SKILL.md, turning a repeated workflow into a reusable skill, or organizing scripts, references, and assets for a skill.
---
...
the agent reads REDLINING.md only when the user needs that feature.
...
- "I can imagine users asking for things like 'Remove the red-eye from this image' or 'Rotate this image'. Are there other ways you imagine this skill would be used?"
...
Create user-installed skills under `<workspace>/skills/<skill-name>/` using the write_file and exec tools. For example, write the initial file to `<workspace>/skills/my-skill/SKILL.md`:

```markdown
---
name: my-skill
description: Describe what this skill does and when PuruClaw should use it.
---
...
The description is the primary triggering mechanism: include both what the skill does and the specific triggers and contexts for when to use it.
...
Create only the directories the skill actually needs. After creating the directory, customize `SKILL.md` and add resources as needed.
