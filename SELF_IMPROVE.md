...
## Slot Ronde 20 (diganti tugas KHUSUS — dikerjakan)
Tugas khusus: tambah command Telegram /stop yang menghentikan agent yang sedang berjalan untuk user tersebut.

## Implementasi Terpilih (Ronde 20)
Tambah command Telegram `/stop` di `internal/app/app.go`: busy-guard per-user kini menyimpan `*busySession` (pointer struct berisi `context.CancelFunc` — pointer agar `CompareAndDelete` aman, func value tak bisa `==`); `Handle` membuat `rctx` cancelable per pesan dan `processMessage` jalan di atasnya; `/stop` via `stopUser` memanggil cancel + melepas busy (`CompareAndDelete` atas nilai ter-Load agar tak menghapus sesi baru yang mulai tepat setelah Load), balas `⏹️ Proses dihentikan.` atau `Tidak ada proses yang berjalan.` bila idle. `releaseCancel` hanya melepas entry milik sesi sendiri (goroutine lama tak menendang sesi baru). Di `internal/ai/exec.go`, `runExec` kini terima `parent context.Context` (dari tool ctx = request ctx): cancel user membunuh grup proses run blocking langsung via watcher `select ctx.Done → killGroup` (bukan nunggu timeout maks 300 dtk), flag `TimedOut` tetap false agar diagnosis benar; sesi background dilepas dari parent (`context.WithoutCancel`) — hidup lintas request, matikan via `kill`. Menu `botCommands` (`internal/telegram/telegram.go`) 3 → 4 (+stop) + teks `/help`. Teruji: `gofmt` bersih, `go vet ./...` bersih, test baru `TestStopUserIdle` + `TestStopUserCancelsAndReleases` (idle false, cancel+lepas, sesi lama tak tendang sesi baru) di `internal/app/app_test.go` + `TestRunExecParentCancelStopsBlocking` + `TestRunExecBackgroundSurvivesParentCancel` di `internal/ai/exec_cancel_test.go` — `go test ./internal/app/ ./internal/telegram/ ./internal/ai/` OK. Dok: AGENTS.md + README.md sinkron. (Selesai)

## Slot Ronde 21 (diganti tugas KHUSUS — dikerjakan)
Tugas khusus: di grup Telegram bot diam kecuali dipanggil via /ai.

## Implementasi Terpilih (Ronde 21)
Gerbang grup di `internal/app/app.go` `Handle`: bila `msg.Chat.Type` group/supergroup dan teks bukan command instan (`/help /clear /token /stop`) dan bukan `/ai`/`/ai@bot` (via `parseAICommand`, `/aid` dkk ditolak) → return nil diam total (tanpa balasan busy sekalipun). `/ai <teks>` kupas prefix, sisa jadi userMessage lewat jalur busy-guard + `processMessage` normal; `/ai` kosong → balas `Kirim /ai <pertanyaan>.` (nil-safe bila `tg` nil). Private chat tetap semua teks diproses. `isCommand` tetap 4 command instan; teks `/help` + baris grup `/ai`. Teruji: `gofmt` bersih, `go vet ./internal/app/ ./internal/telegram/` bersih, `go test ./internal/app/ -count=1` OK + test baru `TestIsCommandIgnoresAI` + `TestIsGroupChat` + `TestParseAICommand`. Dok: AGENTS.md + README.md sinkron. (Selesai)
